...
2. **The log** (console, interleaved with the plain stderr text above, or
   ...
   four records at once, so their log lines interleave in time; the stderr
   `Record '<TaskId>' failed` lines are written after every record has
   finished, in input order.
