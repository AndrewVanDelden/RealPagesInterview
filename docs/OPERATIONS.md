...
| `--model-call-budget-ms <n>` | no (default: the strictest `p95_latency_ms` the batch states) | With `--composer openai` only: what one model call, its one retry included, may take, in place of the budget D28 derives from the records (D70). A positive whole number of milliseconds; anything else, or the flag on any other composer, exits 1. It exists for evaluation runs: on the three sets here the records state 2000 ms, which D28 splits into two 1000 ms attempts, and a completion measured 2 to 4 seconds on 2026-09-10, so without the flag the model answers nothing and every message falls back to the template. The scorecard's p95 check still reads the records' own budget, so a run under the flag reports its p95 against 2000 ms and fails it, which is the true cost of the model rather than a hidden one. |
...
3. **`--diagnostics`**, only if the question is "why did the agent decide
   ...
   spent, `null` for a run that made no network call at all (D28), and
