...
| Call to action payload | sms carries numbered options in the body and `cta.options`; email carries `cta.link`, built from the property slug, the catalog's path and, for a renewal page, the unit, and the scorer compares it with the label's link | options | link | link hosts unseen beyond one; A10, A21, A26, D25, D94, D97 |
...
call-to-action payload by channel, options on sms and on email a link equal to the label's link when the label states one (D94); the opt-out
