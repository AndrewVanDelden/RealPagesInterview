...
6. Whether a live model can meet the records' own `p95_latency_ms` of 2000 ms. The one run bounded
   ...
   runs only (A22).
7. The vendor's rate limit for concurrent calls, against which every record in flight at once is
   unmeasured (A23).
...
| A23 | Every record in flight at once stays under the vendor's rate limit, and a 429 past it is retried once | D123 removes D37's limit of four on the owner's instruction. Neither limit was measured; a batch past the vendor's per-minute limit sees 429s, each retried once before the record falls back to the template | no |
