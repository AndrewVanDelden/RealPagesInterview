...
| Call to action payload | sms carries numbered options in the body and `cta.options`; email carries `cta.link`, built from the property slug, the catalog's path and, for a renewal page, the unit, and the scorer compares the options and the link with the label's | options | link | link hosts unseen beyond one; A10, A21, A26, D25, D94, D97, D100 |
...
| Language | one language per message: `input.language` resolved by primary subtag against the full sets, English, Spanish and French, and any other tag written wholly in English with `locale_applied` false on both composers; the model is told the resolved language, the same one the code-written options and opt-out are in | en | en | A13, D26, D127 |
...
6. Whether a live model can meet the records' own `p95_latency_ms` of 2000 ms. The one run bounded
   ...
   runs only (A22).
7. Whether this run is the only user of the key's rate limits while it runs, which the rate-limit
   gate assumes (A23).
...
| `SafetyValidator` | five checks answering for themselves: opt-out instructions, Social Security number, long digit run, fair-housing terms, and an offer the property system did not state; violations by check (D38, D39, D130) | `ISafetyValidator` (real plus fixed) |
...
to the day and to the hour in the label's offset; `next_action` exact on its type and every member the label states (D99), and beside it the judge's own
...
call-to-action payload by channel, on sms the options and on email the link, each equal to the label's when the label states it (D94, D100); the opt-out
...
| A6 | Unknown or absent timezone resolves to the zone of the US state `city_interest` ends with ("Dallas, TX"), and to UTC when it names none, with a diagnostic either way (D117) | both samples America/Chicago; the field is a free string; `v2_unknown_timezone`'s "America/Dallas" with "Dallas, TX" is labeled Central time. Phase 7 confirmed the UTC fallback on data: `synthetic_08_unknown_timezone` reads Day and Hour OK on every scorecard in `docs/scorecards/`, and FAULT_INJECTION.md section 4 (b) names the diagnostics and tests | no |
| A7 | Horizon = `move_date_target` minus the reference date; at most 60 days is short (`start_cadence`), else long (`follow_up_in_days` 3); an absent date is its own branch, no move date, which a catalog row states separately and the generic row answers with its long action (D102); a past date takes the no-move-date branch, and a record on that branch whose `primary_cta` is stated takes its row's short action where the row states one (D117) | 32 days gave `start_cadence`, 68 gave `follow_up_in_days` 3; `synthetic_v2.jsonl`'s 60- and 61-day records fix the boundary at 60 inside (32, 68], its past-date record follows up like an undated one, and its undated prospects naming `book_tour` take the short cadence where the hold-out's naming none takes the long one (D117); the hold-out's prospect/open record with no date follows up in 2 where sample 2's dated one does in 3, and lead qualification starts from the move timeline (D102) | no |
| A8 | Unknown persona or stage uses the generic row: A7's rule, diagnostics name the fallback. The catalog names `start_cadence`, `follow_up_in_days`, `no_op`, `reset_cadence`, `schedule_sms_reminder`, `branch_on_intent` and `start_esign_flow`. A row states only the horizon branch a labeled record showed, and the other falls to the generic row rather than to an invented value (A19). A record whose persona is not a prospect, an absent one included, takes the generic row's long action on every branch, since its short action names a prospect cadence (D109) | the two samples set prospect/new short and prospect/open long; since D81 the hold-out sets prospect/new long and seven rows, each from one record with no move date; the generic row's short action is a prospect cadence, and no evidence shows a cadence for any other persona | the catalog file, `src/Agent/Decisions/ActionCatalog.cs` (D17) |
| A9 | Call-to-action type: `primary_cta` through the vocabulary table (`book_tour` to `schedule_tour`); unknown values pass through unchanged with a diagnostic, with no email link and, on sms, the language set's generic reply list, which leaves out the tour for a record that is not a prospect (D109); absent gives the generic `reply`; the model path is held to the same type, which its response schema allows alone (D73); an absent `primary_cta` takes the persona and stage's default where one exists (`schedule_tour` for prospect/new, `review_renewal_details` for resident/renewal_details_requested, D82) | one pair in both samples; absence unseen. Phase 7 refuted leaving the type to the model when the record states none: `synthetic_05` reads CTA FAIL in `docs/scorecards/synthetic_12_openai_run1.txt` and `_run2.txt`, where the model chose `contact`, and OK in `_run3.txt` only because the template answered (VARIANCE.md, per-record table); after D73 it reads OK in all three `synthetic_12_openai_after_d72_run*.txt` | the catalog file |
| A10 | sms carries numbered reply options in the body and `cta.options`, joined with a semicolon, and on both composers code writes the options sentence into the body, never the model (D114); a tour invitation's options are the next two open tour slots from two days after the local send date, Monday to Saturday at 10:00 unless the property data's calendar says otherwise, each written `MMM dd, yyyy, h:mm AM` in every language (D108); email carries `cta.link` built as `https://{property slug}.example/{cta path}` | sample 1 options Thu, Fri, two and three days after its Tuesday send; sample 2 link `https://oakridge.example/tour` from Oak Ridge Apartments; lead time and open days from the research of D108 | no |
...
| A13 | A message is written in one language: `input.language` is resolved by primary subtag against the languages with a full set, English, Spanish and French, and any other tag is written wholly in English with the diagnostic `locale_not_applied`, on the model path as on the template path (D127) | both samples en; the synthetic set's item 4 record is es; the field is a free tag, so other values must not fail (D26). The step 99 run of 2026-09-16 showed why the model path cannot pass the tag through: a `fr` record got French prose with English options and opt-out, and a `tlh` record was written in Klingon (D127) | the template sets |
...
| A17 | Required members are `task_id` and `channel_preferences`; a line missing one is an error row naming it; `consent` is optional and an absent or null consent object is consent to nothing, so the record is answered `no_op` with `no_contact_consent` and `consent` is named in its defaulted fields (D125); every other member is optional with its default named in diagnostics; a line that does not parse is also one `ERROR` row of the scorecard, counted in the overall count and measured on no check (D71) | section 2; no decision can be made without the two. D125: a record with no consent object was refused as a line, which wrote no decision for that person, where the safe decision is do not contact. Phase 7: `docs/scorecards/synthetic_12_template.txt` line 14 reads `ERROR: Line 11 failed to parse`, under `Overall: 12/13 passed`, which `tests/Agent.Tests/Evaluation/BaselineNumbersTests.cs` pins (line 33) | no |
...
| A19 | `holdout_12.jsonl` was an evaluation set until 2026-09-10 and is training data from then; `synthetic_12.jsonl` is a regression set; `synthetic_v2.jsonl` was the evaluation set until 2026-09-14 and is training data from then (D116, D117); the owner's larger dataset is the evaluation set, unseen until it is run once | requester's answer, D9; amended by D81 | no |
...
| A21 | The email link is `https://{slug}.example/{path}`: the slug is the property name lowercased with non-alphanumeric characters removed and a trailing property-type word dropped, and the path comes from the catalog's call-to-action column, where a renewal path names the unit (A26, D97); a call to action the column does not name has no path and gets no link, as an absent property name does (D109) | sample 2's `https://oakridge.example/tour` from "Oak Ridge Apartments" and `book_tour`; one pair, so "the first two words concatenated" fits it equally and this picks the type-word rule because the other breaks on a one-word or four-word name (D25) | the catalog file |
| A22 | A model call, the composer's and the judge's, gets 60 seconds given whole to one attempt, and a timeout is never retried (D33, D35, D123); `--model-call-budget-ms` replaces the composer's for a run, and the p95 check keeps the records' own budget (D70) | added from Phase 7, which bounded calls by the records' budget: with one 2000 ms attempt the model wrote 2 of 10 messages, p95 2066 ms (`docs/scorecards/synthetic_12_openai_no_flag_after_d35.txt`); with the flag at 30000 ms it wrote all 10 at 1.5 to 4.5 seconds a record (VARIANCE.md, After D72 and D73); the step 99 run at 2000 ms ended 14 of 48 calls at the timeout (D122's run fact). D123 sets 60 seconds on the owner's instruction | the flag, with `--composer openai` only (D70) |
| A23 | Each model's calls stay within ninety percent of the per-minute request and token limits the vendor reports in its `x-ratelimit-limit-*` headers, counted over the last sixty seconds, and this run is the only user of the key's limits while it runs (D124) | D123 removed D37's limit of four on the owner's instruction and D124 replaced the unmeasured risk with a gate that reads the key's own limits, which depend on its usage tier; Tier 1 is 500 requests and 200,000 tokens a minute on `gpt-4o-mini` and 500 requests and 30,000 tokens on `gpt-4o` (OpenAI rate-limit guide and model pages, read 2026-09-16). Another user of the same key can still cause a 429, which is retried once | no |
...
| A26 | A link path that names the unit takes the unit with every dash character written as a hyphen and every character other than an ASCII letter, digit or hyphen removed; a record with no usable unit gets no link on that path | the hold-out's `A‑204`, written with a non-breaking hyphen, is labeled `/renewal/A-204` and `/renewal/A-204/details`; one unit, so what happens to a space or a slash is a stated choice with no label behind it (D97) | the catalog file |
| A27 | Tour availability, extended tour hours, starting prices and renewal offers come only from `--property-data`, the stand-in for the property management system, and reach only the call to action and the record they serve (D104): a tour invitation's availability only when the record names it in `primary_cta` and has no schedule-conflict cancellation (D111), its hours only after one, its price only for a stated budget; a renewal offer's price hold and text-reminder offer are written by code in the record's language and never given to the model (D112); a price is only ever the total monthly price with every mandatory fee included; the hold-out's file states the label's studio figure as that total, since the label gives no fee breakdown | research of D101: RealPage's leasing agent quotes the property system's pricing and availability, and the FTC's Greystar order requires the all-in monthly price; `holdout_12_property_data.json` is written from three hold-out labels (D101) | the file |
...
| 16 Links, the judge's reason and cost, and the prompt's facts (landed 2026-09-14) | an empty-body completion counted with its tokens (D92); a payload check that compares the label's link and its options and an action check that compares every stated member (D94, D99, D100); the judge's reason and tokens kept (D95); a prompt carrying the stage's facts, the link, the reply options and each call to action's purpose (D96, D103, D105); resident links built from the record (D97); a no-choice call keeping its retries (D98); property facts from `--property-data`, chosen per call to action (D101, D104); no move date as its own branch (D102) | the hold-out run of 2026-09-11 scored every decision 12 of 12 and failed every body on the judge, which is what opened D94 to D105; PR #36's eighteen review findings triaged, fourteen fixed test first, among them a judge fault that crashed the batch and a linear property scan, and three declined; 919 tests at 100 percent |
| 17 The blind set read, then fitted (landed 2026-09-16) | the blind set's 24 flipped checks read record by record, 19 product gaps and 5 label choices (D107); tour options as dated slots from the send date (D108); a record that is not a prospect given no prospect cadence and no tour option, and an unknown call to action no link (D109); the move timeline on email only (D110); tour availability only to a named invitation (D111); renewal terms written by code (D112); the judge given each message's send time (D113) and code the sms options sentence (D114); `synthetic_v2.jsonl` taken as training data and twelve rules fitted from its labels (D116, D117); two prompt-path bugs the model runs exposed (D118) and four a second review of the merged PR #36 found (D119) | `synthetic_v2.jsonl` 26 of 30 from 1, its three remaining failures the labels this project declined, with `sample.jsonl` and `holdout_12.jsonl` unmoved; hold-out bodies 7 to 8 of 11; Phase 4's check passed again on 2026-09-15 at 116 and 916 tests, 100 percent line, branch and method, all four sets offline on the template composer at their pinned tallies |
