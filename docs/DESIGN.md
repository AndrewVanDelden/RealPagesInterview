...
| A27 | Tour availability, starting prices and renewal offers come only from `--property-data`, the stand-in for the property management system, and a price is only ever the total monthly price with every mandatory fee included; the hold-out's file states the label's studio figure as that total, since the label gives no fee breakdown | research of D101: RealPage's leasing agent quotes the property system's pricing and availability, and the FTC's Greystar order requires the all-in monthly price; `holdout_12_property_data.json` is written from three hold-out labels (D101) | the file |
