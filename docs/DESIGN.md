...
| A7 | Horizon = `move_date_target` minus the reference date; at most 45 days is short (`start_cadence`), else long (`follow_up_in_days` 3); an absent date is its own branch, no move date, which a catalog row states separately and the generic row answers with its long action (D102); a past date is short (the move is due) | 32 days gave `start_cadence`, 68 gave `follow_up_in_days` 3; 45 falls inside the open interval (32, 68]; it is not the midpoint (50), just a round number in range; a past date is unseen; the hold-out's prospect/open record with no date follows up in 2 where sample 2's dated one does in 3, and lead qualification starts from the move timeline (D102) | no |
