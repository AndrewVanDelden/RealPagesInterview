...
property facts that serve it, and a language model writes only the sentences around them. Run four
...
Last model result, 2026-09-13, `sprint-16` at `f313f98`: `holdout_12.jsonl` with the model, the
judge and its property data, 12 of 12 with bodies 8 of 11. Last template result, 2026-09-14,
`sprint-17` with dated tour slots, reference times as documented: `sample.jsonl` 2 of 2,
`holdout_12.jsonl` 12 of 12, `synthetic_12.jsonl` 5 of 13 and `synthetic_v2.jsonl` 1 of 30, each
...
the diagnostics file, the review queue and the evaluation report switched on, except steps 3 and 4:
D108 swapped their order and changed the reply options, and their values are from a template run on
2026-09-14 on `sprint-17`.
...
- **Step 3, `src/Agent/Decisions/SendScheduler.cs` and `src/Agent/Decisions/SendSlotTable.cs`, then
  ...
  invitation's reply options count from the send time: two days after Tuesday 2025-12-09 is
  Thursday, the property data holds no calendar, so Monday to Saturday are open, and the two tour
