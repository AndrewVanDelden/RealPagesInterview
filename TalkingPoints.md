...
- D28 (Sprint 6): a model call gets one retry on the SDK's backoff, a per-call timeout taken from the strictest `p95_latency_ms` the batch states, and a temperature of 0.2 that is never called determinism, with the retries visible as `composition.network_retries` (A15).
