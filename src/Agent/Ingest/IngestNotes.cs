...
public sealed record IngestNotes(IReadOnlyList<string> DefaultedFields, IReadOnlyList<string> UnknownMembers)
{
    ...
    public static IngestNotes Describe(ProspectCase prospectCase)
    {
        ...
        NoteAbsent(defaulted, Presence.IsAbsent(context.Unit), "input.unit");
        NoteAbsent(defaulted, context.MoveInDate is null, "input.move_in_date");
        NoteAbsent(defaulted, context.LeaseEndDate is null, "input.lease_end_date");
