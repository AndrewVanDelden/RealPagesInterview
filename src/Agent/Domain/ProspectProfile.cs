    ...
    IReadOnlyList<string>? FeaturesEnablement = null) : HasUnknownMembers
{
    ...
    // characters, other and modifier symbols (where emoji are), marks with no letter before them, and
    // unassigned or private code points. Letters, digits and punctuation are kept.
    ...
    // model's prompt, the ingest notes and the scorer agree on who is greeted.
    ...
    private static bool IsKept(string cluster)
    {
        ...
        return !TrimmedCategories.Contains(Rune.GetUnicodeCategory(first));
    }
