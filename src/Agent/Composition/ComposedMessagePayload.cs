...
// There is no link member. The link is code-owned (A21): code makes every decision that can
// be reproduced and the model writes only prose, so the schema never offers the model the
// field and a hallucinated host cannot reach an email.
